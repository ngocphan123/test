	...
	<xsl:template match="/">
		<html>
			<head>
				...
				<style type="text/css">
					body{background:#fff;border:0px;margin:10px;font:normal 80%
					Verdana,Arial,Helvetica;color:#000}
					hr{color:#C0C0C0;background:#C0C0C0;border:0px
					none;height:1px;clear:both;margin:15px 0}
					...
					.banbox{width:100%;position:fixed;top:0;left:0;background:#A3CEF1;border-bottom:2px
					solid #ccc;height:60px}
					...
					.display-table > div >
					a{display:table-cell;font-size:18px;font-weight:bold;color:#205FA0;text-decoration:none}
				...
				<div class="display-table">
					<div class="head">
						<a href="{link}" class="logo">
							<img height="{$imageHeight}" width="{$imageWidth}" alt="{$imageTitle}"
								src="{$imageUrl}" align="left" />
						</a>
						<a href="{link}" class="middle">
							-
							<xsl:value-of select="$title" />
						</a>
					</div>
