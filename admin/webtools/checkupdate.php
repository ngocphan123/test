        ...
        if (!empty($error)) {
            ...
            foreach ($exts as $extname => $values) {
                ...
                $tooltip[] = array(
                    ...
                    'content' => (!empty($value['version']) ? $value['version'] : 'n/a') . (!empty($value['date']) ? ' (' . nv_date('d/m/Y H:i', strtotime($value['date'])) . ')' : '')
                );
                $tooltip[] = array(
                    'title' => $lang_module['onlineVersion'],
                    'content' => (!empty($value['new_version']) ? $value['new_version'] : ((!empty($value['version']) and $value['origin']) ? $value['version'] : 'n/a')) . (!empty($value['new_date']) ? ' (' . nv_date('d/m/Y H:i', strtotime($value['new_date'])) . ')' : '')
                );
                
                if (!empty($value['author'])) {
                    $tooltip[] = array(
                        'title' => $lang_module['extAuthor'],
                ...
                if (!empty($value['license'])) {
                    $tooltip[] = array(
                        'title' => $lang_module['extLicense'],
                        'content' => $value['license']
                    );
                }
                
                if (!empty($value['mode'])) {
                    $tooltip[] = array(
                        'title' => $lang_module['extMode'],
                        'content' => $value['mode'] == 'sys' ? $lang_module['extModeSys'] : $lang_module['extModeOther']
                    );
                }
                
                if (!empty($value['link'])) {
                    $tooltip[] = array(
                        'title' => $lang_module['extLink'],
                        'content' => "<a href=\"" . $value['link'] . "\">" . $value['link'] . "</a>"
                    );
                }
                
                if (!empty($value['support'])) {
                    $tooltip[] = array(
                        'title' => $lang_module['extSupport'],
                        'content' => "<a href=\"" . $value['support'] . "\">" . $value['support'] . "</a>"
                    );
                }
