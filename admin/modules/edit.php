...
foreach ($data['theme'][2] as $tm) {
    $xtpl->assign('THEME', array(
        'key' => $tm,
        'selected' => $tm == $data['theme'][3] ? ' selected="selected"' : ''
    ));
...
if (!empty($data['mobile'][2]) or !empty($data['mobile'][1])) {
    foreach ($data['mobile'][1] as $tm) {
        $xtpl->assign('MOBILE', array(
            'key' => $tm['key'],
            'selected' => $tm['key'] == $data['mobile'][3] ? ' selected="selected"' : '',
            'title' => $tm['title']
        ));
    ...
    foreach ($data['mobile'][2] as $tm) {
        $xtpl->assign('MOBILE', array(
            'key' => $tm,
            'selected' => $tm == $data['mobile'][3] ? ' selected="selected"' : '',
            'title' => $tm
        ));
        $xtpl->parse('main.mobile.loop');
    }
